"""
Characterization (golden) tests — the kept engines, now from the `reservoirs/` package.

Proves the Phase-2 "move into the package + shim" changed NO numbers: the package
reproduces the Phase-1 goldens (generated from the original pre-package code) bit-for-bit.
Also exercises the new shim surface (rhow alias, forward(), resize_method) and the new
ErdosRenyiReservoir. Runs under `pytest tests/` or standalone.

The archived OLD connectome engine has its own goldens, checked in test_legacy_old_engine.py.
"""
import os
import sys
import warnings
...
# --- golden reproduction (proves the package == original behavior) -----------
...
def test_old_new_native_rng_identical_static():
    """Historical invariant (golden-file level): the archived OLD engine and the NEW engine
    drew identical Win/leak at native size+seed — which justified the back-compat shim.
    Checked statically against the committed goldens so it survives the old engine's archival."""
