...
Citations underpinning the methods in this repo. (Compiled in Phase 4; primary sources preferred.
The decimals quoted from Cossu et al. 2021 in the design doc are approximate — verify against the
ESANN PDF before quoting in a paper.)

> **Implemented vs planned.** Entries tagged **[planned]** back methods that are *designed but NOT
> implemented* (see the status table in `CONTINUOUS_LEARNING_DESIGN.md`). What the code actually
> implements: leaky-integrator ESN; ridge + online RLS/LMS/NLMS readouts; a per-class conceptor
> bank; degree-preserving (Maslov–Sneppen) nulls; GEM + RWalk CL metrics.
...
- Lukoševičius, M. (2012). *A Practical Guide to Applying Echo State Networks.* In *Neural Networks: Tricks of the Trade*. — input scaling, spectral radius, leak, ridge readout, washout.
...
- Yildiz, I.B., Jaeger, H., Kiebel, S.J. (2012). *Re-visiting the echo state property.* Neural Networks 35:1–9. — ρ<1 is necessary-not-sufficient; ESP can hold for ρ>1 under input drive (justifies super-critical spectral radius on short driven windows).
...
- Suárez, L.E. et al. (2024). *Connectome-based reservoir computing with the conn2res toolbox.* Nature Communications 15:656. — directed/undirected connectomes, ridge readout, **null-model (rewired) baselines**, α swept across the critical point.

## Online / incremental readouts
- Jaeger, H. (2003). *Adaptive Nonlinear System Identification with Echo State Networks.* NIPS 15. — RLS readout for ESNs.
...
- Haykin, S. (ed.) (2001). *Kalman Filtering and Neural Networks.* Wiley. — Kalman = RLS with process noise. **[planned]** (no Kalman readout implemented).

## Continual / lifelong learning
- Cossu, A., Bacciu, D., Carta, A., Gallicchio, C., Lomonaco, V. (2021). *Continual Learning with Echo State Networks.* ESANN 2021 (arXiv:2105.07674). — a frozen reservoir does NOT by itself prevent forgetting; SLDA/Replay strong, EWC weak in class-IL.
...
- Chaudhry, A., Dokania, P., Ajanthan, T., Torr, P. (2018). *Riemannian Walk for Incremental Learning.* ECCV. — Forgetting / Intransigence.
